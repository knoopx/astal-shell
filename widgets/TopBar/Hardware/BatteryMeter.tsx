...
export default () => {
  ...
  const tooltipMarkup = percentage((p) => {
    ...
    if (c && ttf > 0) markup += `<b>Time to full:</b> ${formatTime(ttf)}\n`;
    if (cap > 0) markup += `<b>Capacity:</b> ${Math.round(cap)}%\n`;
    if (en > 0 && enFull > 0)
      markup += `<b>Energy:</b> ${en.toFixed(1)}/${enFull.toFixed(1)} Wh\n`;
    if (enRate > 0) markup += `<b>Power draw:</b> ${enRate.toFixed(1)} W\n`;
    if (vol > 0) markup += `<b>Voltage:</b> ${vol.toFixed(2)} V\n`;
    if (temp > 0) markup += `<b>Temperature:</b> ${temp.toFixed(1)}°C\n`;
    if (tech) markup += `<b>Technology:</b> ${tech}\n`;
    if (vend && mod) markup += `<b>Device:</b> ${vend} ${mod}`;
  ...
  return (
    <box tooltipMarkup={tooltipMarkup}>
      <LabeledMeter invert value={percentage} label="BAT" />
    </box>
  );
};
